...
const mutations = {
  updateSelectedShifts(state, { freshShifts, isPastShift }) {
    const currentList = isPastShift ? "selectedPastShifts" : "selectedFutureShifts";
    const oppositeList = isPastShift ? "selectedFutureShifts" : "selectedPastShifts";
    ...
    freshShifts.forEach(fresh => {
      const indexInOpposite = state[oppositeList].findIndex(s => s.id === fresh.id);
