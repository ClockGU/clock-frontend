      ...
      <v-card>
        <v-card-text>
          <v-layout row wrap align-center>
            ...
            <v-flex xs12 md5>
              <shift-form-date-time-input v-model="shift.date" :shift="shift" type="start"/>
            </v-flex>
            <v-flex xs1>to</v-flex>
            <v-flex xs12 md5>
              <shift-form-date-time-input v-model="shift.date" :shift="shift" type="end"/>
            </v-flex>
            <v-flex xs12 md7>
...
export default {
  ...
  data: () => ({
    select: null,
    contracts: [{ uuid: "1234", name: "Starker Start" }]
  }),
  props: {
    uuid: {
      type: String,
